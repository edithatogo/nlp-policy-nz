...
def _markdown_protocol(
    ...
    lines = [
        ...
        "",
        "- GitHub Actions is the default automation surface for validation, packaging, and release checks.",
        "- The CLI is the primary human operator surface, while the HTTP API, Python SDK, and MCP surface provide programmatic and agent-facing access.",
        "- Adjacent datasets and downstream policy-engine consumers should treat exported artifacts as the interchange format rather than depending on private internals.",
        "",
        "## Comparative tooling and runtime choices",
        "",
        "- LanceDB is the default local vector store because it keeps CI and local reproduction simple.",
        "- Qdrant remains an optional service backend for deployments that explicitly need remote vector-service semantics.",
        "- Polars and Arrow-backed transforms should be described as the default tabular-analysis path when they are used.",
        "- Generic ingestion libraries such as Unstructured should be framed as optional future adapters, not as required paper dependencies, unless they are actually committed and reviewed in-tree.",
        "- Mojo, vLLM, or other experimental acceleration paths should be presented as staged options, not as prerequisites for the repository's baseline results.",
        "",
        "## Data and code availability",
        "",
        "- The repository ships the core code, checks, fixtures, generated manuscript artifacts, publication protocol artifacts, and interface contracts required to reproduce the current evidence-bounded results from a clean checkout.",
        "- Optional backends and accelerators remain behind feature detection or environment-specific extras, and downstream datasets or full-corpus inputs are only available when they are explicitly checked in and referenced by the relevant track evidence.",
        "",
        "## Decision appendix",
        "",
