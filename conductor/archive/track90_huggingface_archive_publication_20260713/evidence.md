...
The earlier probe above targeted four empty collection repositories and is
retained as historical evidence, not current status. GitHub issue #96 records
subsequent verification of the live public dataset
`edithatogo/fyi-archive-nz`: a 33,217-record manifest, Parquet materialization,
DuckDB distribution, provenance, SBOM, schemas, and metadata artifacts were
present. Issue #96 was then closed. The hosted dataset card still contains
stale wording, but that is tracked in the upstream `fyi-archive` metadata lane
and does not invalidate the materialized publication milestone.
