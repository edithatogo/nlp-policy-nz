...
| # | Task | Status | Commit |
|---|------|--------|--------|
| 1.1 | Add ANN (annotation), D (docstring), TCH (typing imports), YTT (sys.version checks), RET (return) to ruff lint `select` in pyproject.toml | [ ] | |
...
| 1.4 | Add `#[tool.ruff.lint.per-file-ignores]` for tests (relax ANN in test files) | [ ] | |
| 1.5 | Add `ruff format` check to CI pipeline (currently only `ruff check`) | [ ] | |
...
| # | Task | Status | Commit |
|---|------|--------|--------|
| 2.1 | Add `[tool.pyright]` or `[tool.mypy]` strict config to pyproject.toml | [ ] | |
...
| 2.5 | Add pyright/mypy check to CI pipeline | [ ] | |
...
| # | Task | Status | Commit |
|---|------|--------|--------|
| 3.1 | Create `tests/test_smoke.py` — quick checks: spacy loads, imports work, config files parse | [ ] | |
| 3.2 | Create `tests/integration/test_guard_to_syntactic.py` — guard output → syntactic input round-trip | [ ] | |
| 3.3 | Create `tests/integration/test_pipeline_record_roundtrip.py` — PipelineRecord → Parquet → load | [ ] | |
| 3.4 | Create `tests/integration/test_integrations_zenodo.py` — mock Zenodo API wiring | [ ] | |
| 3.5 | Create `tests/e2e/test_pipeline_e2e.py` — full pipeline: sample XML → process → Parquet → search | [ ] | |
| 3.6 | Add hypothesis property-based tests: `test_hypothesis_normalizer.py`, `test_hypothesis_citations.py` | [ ] | |
| 3.7 | Wire mutation tests: configure `mutatest` to run on core modules, add to CI as optional step | [ ] | |
...
| # | Task | Status | Commit |
|---|------|--------|--------|
| 4.1 | Research `uv_build` build backend vs current `hatchling`; test `uv build` on current config | [ ] | |
| 4.2 | If switching: update `[build-system]` in pyproject.toml to use `uv_build` | [ ] | |
| 4.3 | Verify package builds and installs correctly with new backend | [ ] | |
| 4.4 | Document decision in `docs/build_backend.md` | [ ] | |
...
| # | Task | Status | Commit |
|---|------|--------|--------|
| 5.1 | Add `scalene` to pixi.toml dev dependencies | [ ] | |
| 5.2 | Create `tests/test_coverage.py` marker and `.coveragerc` config for pytest-cov | [ ] | |
| 5.3 | Add Codecov step to `.github/workflows/ci.yml` (after test run) | [ ] | |
| 5.4 | Create `scripts/profile_with_scalene.py` — basic CPU/memory profiling script | [ ] | |
...
| 6.2 | Evaluate pydantic v2 for FastAPI server payload validation (currently using raw BaseModel) | [ ] | |
| 6.3 | Document recommendation: keep msgspec for pipeline, optionally add pydantic v2 for API | [ ] | |
...
| # | Task | Status | Commit |
|---|------|--------|--------|
| 7.1 | Add format check to CI: `ruff format --check .` | [ ] | |
| 7.2 | Add type-check step to CI (pyright or mypy) | [ ] | |
| 7.3 | Add coverage + Codecov step | [ ] | |
| 7.4 | Add smoke test step (fast, runs first) | [ ] | |
