...
class TestModelLoadingBehaviour:
    def test_resolve_torch_dtype_branches(self) -> None:
        ...
        assert (
            _resolve_torch_dtype(
                SimpleNamespace(load_in_4bit=True, bnb_4bit_compute_dtype="float16")
            )
            is torch.float16
        )
        assert (
            _resolve_torch_dtype(SimpleNamespace(load_in_4bit=False, load_in_8bit=True))
            is torch.float16
        )
        assert (
            _resolve_torch_dtype(SimpleNamespace(load_in_4bit=False, load_in_8bit=False))
            is torch.float32
        )
    ...
    def test_load_model_success_and_fallback(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "nlp_policy_nz.semantic.model_loader.AutoTokenizer.from_pretrained",
            fake_tokenizer_from_pretrained,
        )
        monkeypatch.setattr(
            "nlp_policy_nz.semantic.model_loader.AutoModel.from_pretrained",
            fake_model_from_pretrained,
        )

        model, tokenizer = load_model(
            model_name="custom/model", quantization="none", device_map="cpu"
        )
        ...
        monkeypatch.setattr(
            "nlp_policy_nz.semantic.model_loader.AutoModel.from_pretrained",
            failing_model_from_pretrained,
        )

        model, tokenizer = load_model(
            model_name="requested/model", quantization="none", device_map="cpu"
        )
    ...
    def test_load_model_failure_raises_model_load_error(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "nlp_policy_nz.semantic.model_loader.AutoTokenizer.from_pretrained", failing_tokenizer
        )
        monkeypatch.setattr(
            "nlp_policy_nz.semantic.model_loader.AutoModel.from_pretrained",
        ...
        with pytest.raises(
            ModelLoadError, match=f"Failed to load fallback model '{FALLBACK_MODEL}'"
        ):
    ...
    def test_unload_model_calls_cpu_and_clears_cache(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            torch.cuda, "empty_cache", lambda: calls.setdefault("empty_cache", True)
        )
