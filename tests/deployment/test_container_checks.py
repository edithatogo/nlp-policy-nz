...
def test_run_container_smoke_checks():
    ...
    def mock_probe(target, timeout):
        if target in {"http://web:8000", "tcp://db:5432"}:
            return True
        if target == "tcp://invalid":
            raise ValueError("Invalid TCP target")
        if target == "http://api.invalid":
            raise requests.RequestException
        if target == "tcp://storage:9000":
            raise OSError
        return False
    ...
    with patch("nlp_policy_nz.deployment.container_checks._probe_target", side_effect=mock_probe):
        ...
        assert results[3] == SmokeCheckResult(
            name="api", target="http://api.invalid", reachable=False
        )
        assert results[4] == SmokeCheckResult(
            name="storage", target="tcp://storage:9000", reachable=False
        )
