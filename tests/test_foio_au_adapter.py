...
from nlp_policy_nz.extraction.foio_au_adapter import (
    ...
    load_australian_ontology_pinned_bundle,
...
def test_router_is_explicit_and_abstains_when_ambiguous_or_unsupported() -> None:
    ...
    assert route_australian_jurisdiction(
        _record("nz", jurisdiction=None, citation_path="nz/foi")
    ).status == ("abstained")
        ...
        build_australian_archive_bundle(
            [
                _record("cth", jurisdiction="Cth").model_copy(
                    update={
                        "source_trace": _record("cth", jurisdiction="Cth").source_trace.model_copy(
...
def test_per_jurisdiction_metrics_include_disagreement_and_abstention() -> None:
    ...
    nsw = next(
        item for item in report.jurisdictions if item.jurisdiction == AustralianJurisdiction.NSW
    )
...
def test_bounded_fixture_is_pinned_and_evaluable() -> None:
    fixture_path = Path("tests/fixtures/foio/australian_evaluation_fixture.synthetic.json")
...
def test_bounded_bundle_artifact_is_schema_valid_and_candidate_only() -> None:
    bundle = FoioAustralianArchiveBundle.model_validate_json(
        Path("tests/fixtures/foio/australian_cth_dry_run_bundle.synthetic.json").read_text(
            encoding="utf-8"
        )
    )
    assert bundle.review_status == "candidate"
...
def test_authentic_au_cth_bundle_is_pinned_and_contains_no_source_or_labels() -> None:
    path = Path("artifacts/foio/au_cth_ontology_pinned_bundle.json")
    bundle = load_australian_ontology_pinned_bundle(path)

    assert bundle.status == "ontology_pinned_bounded_candidate"
    assert bundle.source_capture_run_id == "29922684296"
    assert bundle.frozen_frame_sha256 == (
        "92b290ba32db7f70589202ae0895303856a64ef8258fb9bab9f561bde45eec7d"
    )
    assert bundle.observed_metrics.f1 == pytest.approx(0.9668874172185431)
    assert bundle.restrictions.source_text_included is False
    assert bundle.restrictions.labels_included is False
    assert "example.test" not in path.read_text(encoding="utf-8")
