...
def test_remote_contract_is_pinned_and_restricts_publication():
    ...
    assert gate["capture_authorized"] is True
    assert gate["publication_authorized"] is False
    assert "rights-cleared" in gate["required_gate"]
    assert gate["operator_authorization"]["sampling_window"].endswith("(179 records)")
